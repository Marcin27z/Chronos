    ...
    <div className="border rounded-lg p-6 hover:border-primary transition-colors group">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
          <Button variant="ghost" size="icon" onClick={() => onDelete(task)} aria-label="Usuń zadanie">
            <Trash className="h-4 w-4" />
          </Button>
        </div>
      </div>

      {task.description && <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{task.description}</p>}

      <div className="space-y-2 text-sm">
        <div className="flex items-center justify-between">
          <span className="text-muted-foreground">Następne wykonanie:</span>
          <span className="font-medium">{formatDate(task.next_due_date)}</span>
        </div>

        <div className="flex items-center justify-between">
          <span className="text-muted-foreground">Interwał:</span>
          <span className="font-medium">{formatInterval(task.interval_value, task.interval_unit)}</span>
        </div>

        {task.preferred_day_of_week !== null && (
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">Preferowany dzień:</span>
            <span className="font-medium">{getDayOfWeekLabel(task.preferred_day_of_week)}</span>
          </div>
        )}
      </div>
    </div>
